...
describe("cache_providers and get_provider_for_attempt", function()
    ...
    it("should retrieve provider for a given attempt", function()
        ...
        -- Проверка результата
        assert.is_nil(err)
        assert.are.same("127.0.0.1", provider.ip)
        assert.are.same("example.com", provider.host)
    ...
    it("should return correct providers for attempts 1 to 4 and nil for attempt 5", function()
        ...
        for i = 1, 4 do
            ...
            assert.are.same(resolved_providers[i].ip, provider.ip)
            assert.are.same(resolved_providers[i].host, provider.host)
